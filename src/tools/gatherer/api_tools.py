...
def _transform_experiment_data(experiment: Dict[str, Any]) -> SimplifiedExperiment:
    ...
    simplified = {
        ...
        "status": experiment.get("state", "inactive"),
        "name": experiment.get("name", ""),
        "description": experiment.get("description", ""),
        "tags": experiment.get("tags", []),
        "context": experiment.get("context", ["unknown"])[0] if experiment.get("context") else "unknown",
        "type": experiment.get("type", "simple"),
        "assignment_unit": experiment.get("assignmentUnit", "individual"),
        "group_type": experiment.get("group", "None") if experiment.get("group") else "None",
        "consistency_rule": experiment.get("consistencyRule", "individual"),
        "post_experiment_rule": {
            "rule": experiment.get("postExperimentRule", "continue"),
            "condition_code": experiment.get("revertTo", "None")
        },
        "decision_points": [],
        "conditions": [],
        "filter_mode": experiment.get("filterMode", "includeAll"),
        "inclusion_users": [],
        "inclusion_groups": [],
        "exclusion_users": [],
        "exclusion_groups": []
    ...
    if "partitions" in experiment:
        for partition in experiment["partitions"]:
            simplified["decision_points"].append({
    ...
    if "conditions" in experiment:
        for condition in experiment["conditions"]:
            simplified["conditions"].append({
                "code": condition.get("conditionCode", ""),
                "weight": condition.get("assignmentWeight", 50)
            })
    
    # TODO: Extract inclusion/exclusion data from segments when needed
    # This would require parsing experimentSegmentInclusion/Exclusion
